...
public:
  ...
  bool isPublic(const MachineOperand &MO) const; // TODO: Remove. Dangerous.
  ...
  bool operator==(const PublicPhysRegs &Other) const;
  bool operator!=(const PublicPhysRegs &Other) const { return !(*this == Other); }

  void removeReg(MCPhysReg Reg);
  void removeRegs(const PublicPhysRegs &Other);

  static PublicPhysRegs symmetricDifference(const PublicPhysRegs &A, const PublicPhysRegs &B);

  const LivePhysRegs &getLPR() const { return LPR; }
};
